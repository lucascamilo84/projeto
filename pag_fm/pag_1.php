...
<body>
        <div class="container">
                <div class="row">
                        <?php while ($user_data = mysqli_fetch_assoc($calcado)) { ?>
                                <div class="col-sm-4 text-center">
                                        <img src="../imagens/adidasbreaknet.png" class="img-fluid  ">
                                        <div class="text-center">
                                                <h4><?php echo mb_strimwidth($user_data['nome'], 0, 30, '...'); ?></h4>
                                        </div>
                                        <div class="text-center">
                                                <h5>R$ <?php echo number_format($user_data['preco'], 2, ',', '.'); ?></h5>
                                        </div>
                                        <div class="text-center">
                                                <button type="button" class="btn btn-outline-info">Compra</button>
                                        </div>
                                </div>
                                <div class="col-sm-4 text-center">
                                        <img src="../imagens/adidasbreaknet_branco_bege.png" class="img-fluid" width="120px">
                                        <div>
                                                <h4><?php echo mb_strimwidth($user_data['nome'], 0, 30, '...'); ?></h4>
                                        </div>
                                        <div>
                                        ...
                                        <div class="text-center">
                                                ...
                                                if (isset($_POST['buttonPesquisar']) and $_POST['txtpesquisar'] != "") {
                                                        $nome = '%' . $_POST['txtpesquisar'] . '%';
                                                        $query =  "SELECT * FROM calcados where nome LIKE '$nome'";
                                                } else {
                                                        $query = 'SELECT * FROM calcados';
                                                }
                                                ...
                                                if ($row < 1) {
                                                        echo '<h2>Não foram achados produtos</h2>';
                                                        echo $row;
                                                } else {
                                                        while ($res = mysqli_fetch_array($result)) {
                                                                ...
                                                                if ($user_data['quantidade'] > 0) { ?>
                                                                        <a href="../oi.php?id=<?php echo $res['id']; ?>">
                                                                                <button type="button" class="btn btn-outline-info">Compra</button></a>
                                                                <?php  } else { ?>
                                                                        <button type="button" class="btn btn-light">Em falta</button>
                                                        <?php }
                                                        } ?>
